...
A skill is reusable, on-demand knowledge. Claude Code reads the short description above to
decide whether a task is relevant, and only then loads this body. That keeps the domain
detail out of the default context window until a booking task actually needs it. This
is the difference between a skill and `CLAUDE.md`: `CLAUDE.md` is always loaded, a skill is
